...
#include "wingurpc-demo-packet.h"
...
transport_read(TcpTransport *trans)
{
    ...
    wingurpc_client_generic_callback (pac->data, ret_len, trans->rpc_priv, NULL);
    trans->rpc_priv = NULL;
}
...
strlen_callback(void *vresult, void *user_data, GError *error)
{
    ...
    printf("the length of string 'hello wingurpc' is %d.\n", len);
}
...
main(int argc, char *argv[])
{
    ...
    wingurpc_client_async_call__int(rpc_client, "wingurpc_strlen",
                                  strlen_callback, "user data",
                                  1, "string", "hello wingurpc");
